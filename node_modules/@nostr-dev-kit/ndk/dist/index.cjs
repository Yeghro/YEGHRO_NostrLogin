...
// src/user/index.ts
var import_nip195 = require("nostr-tools/nip19");
...
var NDKRelayConnectivity = class {
  ...
  constructor(ndkRelay) {
    ...
    this.relay = new import_nostr_tools.Relay(this.ndkRelay.url);
...
function filterFromId(id) {
  ...
  if (id.match(BECH32_REGEX)) {
    try {
      decoded = (0, import_nip19.decode)(id);
      switch (decoded.type) {
        case "nevent":
...
function relaysFromBech32(bech322) {
  try {
    const decoded = (0, import_nip19.decode)(bech322);
...
var NDKSubscriptionFilters = class {
  ...
  eventMatchesLocalFilter(event) {
    ...
    return this.filters.some((filter) => (0, import_nostr_tools2.matchFilter)(filter, rawEvent));
  }
};
...
var import_nip192 = require("nostr-tools/nip19");
...
async function generateContentTags(content, tags = []) {
  ...
  content = content.replace(tagRegex, (tag) => {
    try {
      ...
      const { type, data } = (0, import_nip192.decode)(entity);
      let t;
      switch (type) {
        case "npub":
          t = ["p", data];
...
// src/events/nip19.ts
var import_nip193 = require("nostr-tools/nip19");
function encode() {
  let relays = [];
  ...
  if (this.isParamReplaceable()) {
    return (0, import_nip193.naddrEncode)({
      kind: this.kind,
      pubkey: this.pubkey,
    ...
    return (0, import_nip193.neventEncode)({
      id: this.tagId(),
      relays,
      author: this.pubkey
    });
  } else {
    return (0, import_nip193.noteEncode)(this.tagId());
  }
}
...
function getRootEventId(event, searchTags = ["a", "e"]) {
  const rootEventTag = getRootTag(event, searchTags);
  return rootEventTag?.[1];
}
function getRootTag(event, searchTags = ["a", "e"]) {
...
var import_nip57 = require("nostr-tools/nip57");
...
var NDKZap = class _NDKZap extends import_tseep3.EventEmitter {
  ...
  async generateZapRequest(amount, comment, extraTags, relays, signer) {
    ...
    const zapRequest = (0, import_nip57.makeZapRequest)({
...
var NDKUser = class _NDKUser {
  ...
  get npub() {
    if (!this._npub) {
      ...
      this._npub = (0, import_nip195.npubEncode)(this.pubkey);
    }
    return this._npub;
  }
  set npub(npub) {
  ...
  get pubkey() {
    if (!this._pubkey) {
      ...
      this._pubkey = (0, import_nip195.decode)(this.npub).data;
    }
    return this._pubkey;
  }
  /**
...
var import_nip196 = require("nostr-tools/nip19");
  ...
  async getArticle() {
    ...
    switch (articleTag[0]) {
      case "a":
        ...
        taggedBech32 = (0, import_nip196.naddrEncode)({ kind: parseInt(kind), pubkey, identifier });
        break;
      case "e":
        taggedBech32 = (0, import_nip196.noteEncode)(articleTag[1]);
        break;
      case "r":
...
var nip04 = __toESM(require("nostr-tools/nip04"), 1);
var NDKPrivateKeySigner = class _NDKPrivateKeySigner {
  ...
  constructor(privateKey) {
    if (privateKey) {
      ...
      this._user = new NDKUser({
        pubkey: (0, import_pure.getPublicKey)(this.privateKey)
      });
    }
  }
  static generate() {
    const privateKey = (0, import_pure.generateSecretKey)();
  ...
  async sign(event) {
    ...
    return (0, import_pure.finalizeEvent)(event, this.privateKey).sig;
  }
...
async function fetchEventFromTag(tag, subOpts, fallback = {
  ...
  let result = void 0;
  let relay = hint !== "" ? this.pool.getRelay(hint) : void 0;
